...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await importInjuriesFromApiFootball(leagueId, season);

    return NextResponse.json({
      success: true,
      message: "Sakatlık verileri başarıyla içe aktarıldı.",
      result,
    });
  } catch (error) {
    console.error("Sakatlık import hatası:", error);
