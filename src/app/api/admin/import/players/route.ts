...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await importPlayersFromApiFootball(leagueId, season);

    return NextResponse.json({
      success: true,
      message: "Oyuncular başarıyla içe aktarıldı.",
      result,
    });
  } catch (error) {
    console.error("Oyuncu import hatası:", error);
