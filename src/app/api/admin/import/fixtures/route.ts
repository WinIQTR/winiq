...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await importFixturesFromApiFootball(
      leagueId,
      season,
    );
    ...
    return NextResponse.json({
      success: true,
      message: "Maçlar başarıyla içe aktarıldı.",
      result,
    });
  } catch (error) {
    console.error("Fixture import hatası:", error);
