  ...
  return <PageShell><main className={styles.page}>
    ...
    <section className={styles.panel}><header className={styles.panelHead}><h2>{trDate(start)} – {trDate(new Date(end.getTime()-day))}</h2><span className={styles.badge}>{competition.name}</span></header><div className={styles.rows}>
      {matches.length===0?<div className={styles.empty}>Bu filtrede maç bulunamadı.</div>:matches.map((match)=>{
        ...
        return <article className={styles.match} key={match.id}><div className={styles.meta}><strong>{trDate(match.kickoffAt)} · {trTime(match.kickoffAt)}</strong><span>{match.round ?? "Tur bilgisi yok"}</span></div><div className={styles.teams}><div className={styles.team}>{match.homeTeam.logoUrl?<Image src={match.homeTeam.logoUrl} alt="" width={24} height={24}/>:<span/>}<strong>{match.homeTeam.name} {hasHotSupport(match.homeTeam.name)?<span className={styles.hotFans} title="Taraftar atmosferiyle öne çıkan kulüp">🔥</span>:null} {favorite==="HOME"?<span className={styles.favorite} title={`Model favorisi${favoriteProbability!==null?` · %${Math.round(favoriteProbability)}`:""}`}>★</span>:null}</strong><span className="form-result-badge">{homeForm.map((result,index)=><i className="form-result-dot" data-result={result} key={index}>{result==="W"?"G":result==="D"?"B":"M"}</i>)}</span><b className={styles.score}>{match.homeScore ?? "–"}</b></div><div className={styles.team}>{match.awayTeam.logoUrl?<Image src={match.awayTeam.logoUrl} alt="" width={24} height={24}/>:<span/>}<strong>{match.awayTeam.name} {hasHotSupport(match.awayTeam.name)?<span className={styles.hotFans} title="Taraftar atmosferiyle öne çıkan kulüp">🔥</span>:null} {favorite==="AWAY"?<span className={styles.favorite} title={`Model favorisi${favoriteProbability!==null?` · %${Math.round(favoriteProbability)}`:""}`}>★</span>:null}</strong><span className="form-result-badge">{awayForm.map((result,index)=><i className="form-result-dot" data-result={result} key={index}>{result==="W"?"G":result==="D"?"B":"M"}</i>)}</span><b className={styles.score}>{match.awayScore ?? "–"}</b></div><span className={styles.favoriteNote}>{favorite==="DRAW"?`◆ Beraberlik önde${favoriteProbability!==null?` · %${Math.round(favoriteProbability)}`:""}`:favorite?"★ Model favorisi":"Favori verisi bekleniyor"}</span></div><span className={`${styles.status} ${match.status==="LIVE"?styles.live:match.status==="FINISHED"?styles.done:""}`}>{statusLabel(match.status,match.statusShort)}</span><div className={styles.meta}><span>{venue}{match.homeTeam.venueCapacity?` · ${match.homeTeam.venueCapacity.toLocaleString("tr-TR")} kişi`:""}</span><span>{match.referee ?? "Hakem bekleniyor"}</span></div>
