...
export default async function FixturesPage({ searchParams }: Props) {
  ...
  const all = await prisma.match.findMany({
    ...
    include:{homeTeam:{select:{name:true,logoUrl:true}},awayTeam:{select:{name:true,logoUrl:true}},teamStatistics:{select:{teamId:true,shots:true,shotsOnTarget:true,possession:true,corners:true,fouls:true,offsides:true,yellowCards:true,redCards:true,expectedGoals:true}}},
  });
  ...
  return <PageShell><main className={styles.page}>
    ...
    <section className={styles.panel}><header className={styles.panelHead}><h2>{trDate(start)} – {trDate(new Date(end.getTime()-day))}</h2><span className={styles.badge}>{competition.name}</span></header><div className={styles.rows}>
      {matches.length===0?<div className={styles.empty}>Bu filtrede maç bulunamadı.</div>:matches.map((match)=>{
        ...
        return <article className={styles.match} key={match.id}><div className={styles.meta}><strong>{trDate(match.kickoffAt)} · {trTime(match.kickoffAt)}</strong><span>{match.round ?? "Tur bilgisi yok"}</span></div><div className={styles.teams}><div className={styles.team}>{match.homeTeam.logoUrl?<Image src={match.homeTeam.logoUrl} alt="" width={24} height={24}/>:<span/>}<strong>{match.homeTeam.name}</strong><b className={styles.score}>{match.homeScore ?? "–"}</b></div><div className={styles.team}>{match.awayTeam.logoUrl?<Image src={match.awayTeam.logoUrl} alt="" width={24} height={24}/>:<span/>}<strong>{match.awayTeam.name}</strong><b className={styles.score}>{match.awayScore ?? "–"}</b></div></div><span className={`${styles.status} ${match.status==="LIVE"?styles.live:match.status==="FINISHED"?styles.done:""}`}>{statusLabel(match.status,match.statusShort)}</span><div className={styles.meta}><span>{match.venueName ?? "Stat bekleniyor"}</span><span>{match.referee ?? "Hakem bekleniyor"}</span></div>
