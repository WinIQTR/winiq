...
const form = (results:FormResult[]) => <span className={styles.form}>{Array.from({length:5},(_,index)=>results[index] ?? null).map((result,index)=>result ? <i data-result={result} key={index}>{result === "W" ? "G" : result === "D" ? "B" : "M"}</i> : <i className={styles.formMissing} key={index}>—</i>)}</span>;
